  ...
  export default {
    ...
    methods: {
      ...
      sendText() {
        let that = this;
        if(!this.first) {
          this.list = [...this.list, {type: "mine", content: this.model.contentText, avatar: this.user.avatar}];//通过type字段进行区分是自己（mine）发的还是系统（robot）返回的
        }
        this.first = false
      ...
      backText(callback) {
        ...
        if (window.WebSocket) {
          ...
          ws.onmessage = function (e) {
            ...
            console.log(obj.contentText);
            if(obj.user != '2' && obj.contentText != that.message) {
