    ...
    <div class="blog-main">
      ...
      <div class="widget-courselist">
        <ul class="pagelist-wrapper" v-if="items">
          ...
          <a @click="blogDetail(item._id)" class="item-cover" target="_blank">
            <img :src="item.cover">
          </a>
          <div class="item-info" style="width: 450px;">
            <a @click="blogDetail(item._id)" class="item-title" title="用照片讲故事，放弃拍摄大头照，你将得到更多" target="_blank">{{item.title}}</a>
            <p class="item-desc">
              {{item.introduction}}
            </p>
          </div>
      ...
      <aside class="vision-aside">
        <div>
          <h3 class="aside-title">博客反馈</h3>
          <ul class="vision-intro">
            对博客有疑问？
            <a target="_balnk" href="https://wj.qq.com/s/885142/b5b4">
              点击提交
            </a>
          </ul>
        </div>
